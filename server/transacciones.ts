...
function rangoFechas(query: any) {
  ...
  if (periodo === 'hoy') {
    ...
    if (desde) d = new Date(desde);
    if (hasta) h = new Date(hasta);
  }
  return { desde: d, hasta: h };
}

router.get('/', authGuard, async (req, res) => {
  const { tipo, cliente, clienteId, numero, page = '1', limit = '50', sort = 'fecha', order = 'desc' } = req.query as any;
  ...
  if (numero) { where.push('c.numero_cuenta LIKE ?'); params.push(`%${numero}%`); }
  if (clienteId && Number(clienteId)) { where.push('cl.id = ?'); params.push(Number(clienteId)); }
  if (cliente) {
    ...
    for (let i=0;i<4;i++) params.push(`%${cliente}%`);
  }
  ...
  try {
    const [rows] = await pool.query(
      `SELECT t.id, t.fecha, t.tipo, t.monto, t.concepto,
