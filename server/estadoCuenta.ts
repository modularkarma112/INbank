...
import { generateEstadoCuentaPdf, enviarEstadoCuenta, renderEstadoCuentaHtml } from './mailer';

const router = Router();

function parsePeriodo(query: any): { desde: Date; hasta: Date; etiqueta: string } {
  const now = new Date();
...
async function fetchEstadoCuenta(clienteId: number, desde: Date, hasta: Date) {
  // Datos del cliente
  ...
  const movimientosPorCuenta: Record<string, any[]> = {};
  if (cuentaIds.length) {
    ...
    for (const m of movs) {
      ...
      if (!movimientosPorCuenta[key]) movimientosPorCuenta[key] = [];
  ...
  const [prestamos]: any = await pool.query(
...
router.get('/:clienteId', authGuard, async (req, res) => {
  const clienteId = Number(req.params.clienteId);
  if (!Number.isFinite(clienteId)) return res.status(400).json({ message: 'Cliente inválido' });
...
// Descargar PDF
router.get('/:clienteId/pdf', authGuard, async (req, res) => {
  const clienteId = Number(req.params.clienteId);
  if (!Number.isFinite(clienteId)) return res.status(400).json({ message: 'Cliente inválido' });
  ...
  try {
    const data = await fetchEstadoCuenta(clienteId, desde, hasta);
    if (!data) return res.status(404).json({ message: 'Cliente no encontrado' });
    const pdf = await generateEstadoCuentaPdf({ ...data, etiqueta });
...
router.get('/:clienteId/html', authGuard, async (req, res) => {
  const clienteId = Number(req.params.clienteId);
  if (!Number.isFinite(clienteId)) return res.status(400).json({ message: 'Cliente inválido' });
  const { desde, hasta, etiqueta } = parsePeriodo(req.query);
  try {
    const data = await fetchEstadoCuenta(clienteId, desde, hasta);
    if (!data) return res.status(404).json({ message: 'Cliente no encontrado' });
    const html = renderEstadoCuentaHtml({ ...data, etiqueta });
...
router.post('/:clienteId/generar', authGuard, async (req, res) => {
  const clienteId = Number(req.params.clienteId);
  if (!Number.isFinite(clienteId)) return res.status(400).json({ message: 'Cliente inválido' });
  const { desde, hasta, etiqueta } = parsePeriodo(req.query);
  try {
...
router.get('/por-cuenta/:numero', authGuard, async (req, res) => {
  const numero = String(req.params.numero || '').trim();
  if (!numero) return res.status(400).json({ message: 'Número de cuenta requerido' });
  const { desde, hasta } = parsePeriodo(req.query);
  try {
    const [[row]]: any = await pool.query('SELECT cliente_id FROM cuentas WHERE numero_cuenta = ? LIMIT 1', [numero]);
    if (!row?.cliente_id) return res.status(404).json({ message: 'Cuenta no encontrada' });
...
router.get('/por-cuenta/:numero/pdf', authGuard, async (req, res) => {
  const numero = String(req.params.numero || '').trim();
  if (!numero) return res.status(400).json({ message: 'Número de cuenta requerido' });
  const { desde, hasta, etiqueta } = parsePeriodo(req.query);
  try {
...
router.get('/por-cuenta/:numero/html', authGuard, async (req, res) => {
  const numero = String(req.params.numero || '').trim();
  if (!numero) return res.status(400).json({ message: 'Número de cuenta requerido' });
  const { desde, hasta, etiqueta } = parsePeriodo(req.query);
  try {
...
router.post('/por-cuenta/:numero/generar', authGuard, async (req, res) => {
  const numero = String(req.params.numero || '').trim();
  if (!numero) return res.status(400).json({ message: 'Número de cuenta requerido' });
  const { desde, hasta, etiqueta } = parsePeriodo(req.query);
  try {
