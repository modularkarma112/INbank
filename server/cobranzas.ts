...
import { enviarNotificacionPagoPrestamo } from './mailer';
...
router.get('/aprobados', authGuard, async (_req, res) => {
  await ensurePagosTable();
  // Intentar seleccionar creado_en; si no existe, caemos a la consulta sin ese campo
  const baseSelect =
    'SELECT p.id, p.cliente_id, p.tipo_prestamo, p.monto, p.plazo_meses, p.estatus, p.interes, p.interes_penalizacion, c.nombres, c.apellido_paterno, c.apellido_materno';
  try {
    const [rows] = await pool.query<any[]>(
      baseSelect + ', p.creado_en FROM prestamos p INNER JOIN clientes c ON c.id = p.cliente_id WHERE p.estatus = "APROBADO" ORDER BY p.id DESC LIMIT 500'
    );
  ...
  const [rows] = await pool.query<any[]>(
      baseSelect + ' FROM prestamos p INNER JOIN clientes c ON c.id = p.cliente_id WHERE p.estatus = "APROBADO" ORDER BY p.id DESC LIMIT 500'
    );
    const enriched = await attachPagosPeriodo(rows.map(enrichCobranzaRow));
    return res.json(enriched);
  }
...
function enrichCobranzaRow(r: Row) {
...
async function attachPagosPeriodo(items: any[]){
  const period = currentPeriodKey(new Date());
  const ids = items.map(x => x.id);
  if (!ids.length) return items;
  const [pRows]: any[] = await pool.query(
    'SELECT prestamo_id, COALESCE(SUM(monto),0) AS pagado FROM pagos_prestamo WHERE periodo_yyyymm = ? AND prestamo_id IN (?) GROUP BY prestamo_id',
    [period, ids]
  );
  const map: Record<string, number> = {};
  for (const r of pRows || []) map[String(r.prestamo_id)] = Number(r.pagado || 0);
  for (const it of items) {
    ...
    if (isPagado) it.estado_pago = 'PAGADO';
  }
  return items;
}

router.post('/pago', authGuard, async (req, res) => {
  await ensurePagosTable();
  const { prestamo_id, monto } = req.body || {};
  const pid = Number(prestamo_id);
  const amount = Number(monto);
  if (!pid || !(amount > 0)) return res.status(400).json({ message: 'Datos inválidos' });

  const period = currentPeriodKey(new Date());
  await pool.execute('INSERT INTO pagos_prestamo (prestamo_id, monto, periodo_yyyymm) VALUES (?, ?, ?)', [pid, amount, period]);

  // Obtener datos para el email
  const [[row]]: any = await pool.query(
    'SELECT p.id, p.cliente_id, p.plazo_meses, p.monto, p.interes, c.nombres, c.apellido_paterno, c.apellido_materno, c.email FROM prestamos p INNER JOIN clientes c ON c.id = p.cliente_id WHERE p.id = ? LIMIT 1',
    [pid]
  );
  if (row?.email) {
    ...
    const [[pg]]: any = await pool.query('SELECT COALESCE(SUM(monto),0) AS pagado FROM pagos_prestamo WHERE prestamo_id = ? AND periodo_yyyymm = ?', [pid, period]);
    const saldo = Math.max(cuota - Number(pg?.pagado||0), 0);
    enviarNotificacionPagoPrestamo({
