...
exports.enviarNotificacionMovimiento = enviarNotificacionMovimiento;
exports.renderComprobanteMovimiento = renderComprobanteMovimiento;
exports.generateComprobanteMovimientoPdf = generateComprobanteMovimientoPdf;
exports.generateComprobanteTransferenciaPdf = generateComprobanteTransferenciaPdf;
exports.enviarNotificacionSolicitudPrestamo = enviarNotificacionSolicitudPrestamo;
exports.enviarNotificacionPagoPrestamo = enviarNotificacionPagoPrestamo;
exports.enviarEstadoCuenta = enviarEstadoCuenta;
exports.generateEstadoCuentaPdf = generateEstadoCuentaPdf;
exports.renderEstadoCuentaHtml = renderEstadoCuentaHtml;
...
async function enviarNotificacionMovimiento(data) {
  ...
  `;
    try {
        ...
        if (data.comprobantePdf) {
            try {
                const pdfSize = data.comprobantePdf.length;
...
function renderComprobanteMovimiento(opts) {
...
async function generateComprobanteMovimientoPdf(opts) {
    const { tipo, monto, concepto, numeroCuenta, nombreCliente, referencia, fecha } = opts;
    ...
    return new Promise((resolve, reject) => {
        ...
        if (referencia != null)
            doc.text(`Referencia: #${referencia}`);
        doc.moveDown(0.5);
...
async function generateComprobanteTransferenciaPdf(opts) {
    const { id, monto, comision, concepto, fecha, tipo, origen_nombre, origen_numero_cuenta, destino_nombre, destino_banco, destino_identificador_tipo, destino_identificador } = opts;
    const doc = new pdfkit_1.default({ size: 'A4', margin: 50 });
    const chunks = [];
    return new Promise((resolve, reject) => {
        doc.on('data', (d) => chunks.push(d));
...
async function enviarNotificacionSolicitudPrestamo(data) {
    if (!process.env.SMTP_USER || !process.env.SMTP_PASS) {
...
async function enviarNotificacionPagoPrestamo(data) {
    if (!process.env.SMTP_USER || !process.env.SMTP_PASS) {
...
async function enviarEstadoCuenta(data) {
    if (!process.env.SMTP_USER || !process.env.SMTP_PASS) {
...
async function generateEstadoCuentaPdf(data) {
...
function renderEstadoCuentaHtml(data) {
    const { cliente, periodo, cuentas, prestamos, etiqueta } = data;
    ...
    const movimientosTabla = (movs) => {
        if (!movs || !movs.length)
            return '<div style="color:#6b7280">Sin movimientos en el período.</div>';
        return `
